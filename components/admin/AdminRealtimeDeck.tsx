...
// ============================================================================
// ASTITVA 2K26 - Admin Realtime Executive Control Center Deck
// Path: components/admin/AdminRealtimeDeck.tsx
// ============================================================================
...
interface AdminRealtimeDeckProps {
...
interface LiveLogItem {
  id: string;
  type: "REGISTRATION" | "ATTENDANCE" | "USER" | "ANNOUNCEMENT";
  title: string;
  detail: string;
  time: string;
}

export function AdminRealtimeDeck({ initialData, user }: AdminRealtimeDeckProps) {
        ...
        (payload: any) => {
          if (payload.eventType === "INSERT") {
            ...
            const item: LiveLogItem = {
              ...
              detail: `Ticket ${payload.new?.registrationNumber || "issued"}`,
              time: new Date().toLocaleTimeString(),
            };
            setLiveLogs((prev) => [item, ...prev.slice(0, 7)]);
            toast.success("🚨 Live: New event registration received!");
          }
        }
      )
      .on(
        ...
        (payload: any) => {
          ...
          const item: LiveLogItem = {
            id: `att-${Date.now()}`,
            type: "ATTENDANCE",
            title: "Gate Scan Check-in",
            detail: `Participant ${payload.new?.participantId || "Verified"}`,
            time: new Date().toLocaleTimeString(),
          };
          setLiveLogs((prev) => [item, ...prev.slice(0, 7)]);
          toast.info("⚡ Live: Gate scan check-in verified!");
        }
      )
      .on(
        "postgres_changes" as any,
        ...
        (payload: any) => {
          ...
          const item: LiveLogItem = {
            ...
            detail: payload.new?.email || "New Account",
            time: new Date().toLocaleTimeString(),
          };
          setLiveLogs((prev) => [item, ...prev.slice(0, 7)]);
          toast.success("👤 Live: New user signed up!");
        }
      )
      .on(
        "postgres_changes" as any,
        ...
        () => {
          setData((prev) => ({
            ...prev,
            totals: { ...prev.totals, announcements: prev.totals.announcements + 1 },
          }));
        }
      )
      .on(
        "postgres_changes" as any,
        { event: "INSERT", schema: "public", table: "Certificate" },
        () => {
          setData((prev) => ({
            ...prev,
            totals: { ...prev.totals, certificates: prev.totals.certificates + 1 },
          }));
        }
      )
  ...
  return (
    <div className="space-y-8 animate-in fade-in-50 duration-300 text-[#1A1918]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 font-mono">
        <KpiCard icon={<Users className="h-4 w-4 text-[#1A1918]" />} label="Total Users" value={data.totals.users} isLive={isConnected} />
        <KpiCard icon={<UserCheck className="h-4 w-4 text-[#E85A4F]" />} label="Registrations" value={data.totals.registrations} accent="text-[#E85A4F]" isLive={isConnected} />
        <KpiCard icon={<Trophy className="h-4 w-4 text-[#1A1918]" />} label="Tournaments" value={data.totals.events} isLive={isConnected} />
        <KpiCard icon={<Award className="h-4 w-4 text-[#E85A4F]" />} label="Certificates" value={data.totals.certificates} accent="text-[#E85A4F]" isLive={isConnected} />
        <KpiCard icon={<Megaphone className="h-4 w-4 text-[#1A1918]" />} label="Announcements" value={data.totals.announcements} isLive={isConnected} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="p-6 sm:p-7 rounded-3xl bg-[#F6F4EE] border border-[#8E8D8A]/25 shadow-sm space-y-5 font-mono">
            <div className="flex items-center justify-between border-b border-[#8E8D8A]/20 pb-4">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#EAE7DC] border border-[#8E8D8A]/30 text-[#E85A4F] uppercase flex items-center gap-1">
                <Radio className="h-3 w-3 animate-pulse" /> Live Postgres Stream
              </span>
            </div>
          ...
          {/* Live Postgres Stream Activity Log */}
          {liveLogs.length > 0 && (
            <div className="p-5 rounded-3xl bg-[#F6F4EE] border border-[#8E8D8A]/25 shadow-sm space-y-3 font-mono">
              <div className="flex items-center justify-between text-xs font-bold text-[#1A1918] uppercase">
                <span className="flex items-center gap-1.5 text-[#E85A4F]">
                  <Zap className="h-3.5 w-3.5" /> Realtime Activity Stream
                </span>
                <span className="text-[10px] text-[#8E8D8A]">Subscribed to Supabase Channel</span>
              </div>
              <div className="divide-y divide-[#8E8D8A]/15">
                {liveLogs.map((log) => (
                  <div key={log.id} className="py-2 flex items-center justify-between text-xs animate-in fade-in duration-300">
                    <div className="flex items-center gap-2">
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#EAE7DC] text-[#1A1918]">
                        {log.type}
                      </span>
                      <span className="font-bold text-[#1A1918]">{log.title}</span>
                      <span className="text-[#8E8D8A] text-[11px]">{log.detail}</span>
                    </div>
